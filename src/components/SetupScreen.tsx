...
export const SetupScreen: React.FC<SetupScreenProps> = ({ exam, onStartExam, onBack, lang }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-100 dark:border-slate-900">
              <div className="text-center p-2">
                <span className="block text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">{t.setupQuestionCount}</span>
                <span className="block text-2xl font-extrabold text-gray-900 dark:text-white mt-1">
                  {exam.questions.length} <span className="text-xs font-medium text-gray-400 dark:text-gray-500">{t.setupQuestionCountUnit}</span>
                </span>
              </div>
              <div className="text-center p-2 border-l border-slate-100 dark:border-slate-900">
                <span className="block text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">{t.setupQuestionTypes}</span>
                <div className="flex justify-center gap-1.5 mt-2 flex-wrap">
                  {exam.questions.some(q => q.type === 'multiple-choice') && (
                    <span className="px-2 py-0.5 text-[10px] font-bold bg-violet-100 dark:bg-violet-950/40 text-violet-700 dark:text-violet-300 rounded-md">
                      {lang === 'id' ? 'Pilihan Ganda' : 'Multiple Choice'}
                    </span>
                  )}
            ...
            <div className="p-5 rounded-2xl border border-gray-200 dark:border-gray-800 space-y-4 bg-white dark:bg-slate-900">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-sm text-gray-900 dark:text-white">{t.setupDurationLimit}</h4>
                    <p className="text-xs text-gray-400 dark:text-gray-500">{t.setupDurationDesc}</p>
                  </div>
                </div>
              ...
              {enableTimer && (
                <div className="flex items-center gap-3 pt-2 pl-12 border-t border-gray-100 dark:border-gray-800">
                  <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{lang === 'id' ? 'Durasi:' : 'Duration:'}</span>
                  <input
                    type="number"
                    min="1"
                    max="600"
                  ...
                  />
                  <span className="text-sm font-semibold text-gray-500 dark:text-gray-400">{t.setupDurationUnit}</span>
                </div>
              )}
            </div>
            ...
            <div className="p-5 rounded-2xl border border-gray-200 dark:border-gray-800 space-y-4 bg-white dark:bg-slate-900">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-sm text-gray-900 dark:text-white">{t.setupRandomization}</h4>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{t.setupRandomizationDesc}</p>
                </div>
              </div>
      ...
      {/* Footer */}
      <footer className="py-4 text-center text-xs text-gray-450 dark:text-gray-600">
        {lang === 'id' ? 'Pastikan jawaban Anda terisi sebelum waktu habis.' : 'Make sure all answers are filled before time runs out.'}
      </footer>
    </div>
  );
};
