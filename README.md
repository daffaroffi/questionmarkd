...
> **Prinsip Utama**: Upload Markdown &rarr; Start Exam
...
- **Styling**: Tailwind CSS v4 (Sangat cepat, konfigurasi native CSS modern)
- **Icons**: Lucide React
- **Markdown & LaTeX Rendering**:
  - `react-markdown`
  - `remark-gfm` (dukungan tabel dan task lists)
  - `remark-math` + `rehype-katex` (dukungan LaTeX)
- **Interaksi & Animasi**:
  - `canvas-confetti` (selebrasi kelulusan)
...
### Langkah-langkah
1. **Clone repository ini** (atau buka direktori kerja Anda):
   ```bash
...
4. **Buka aplikasi**:
   Buka peramban (browser) dan akses alamat [http://localhost:5173/](http://localhost:5173/).
...
Aplikasi ini berjalan 100% secara lokal pada peramban Anda. Tidak ada data jawaban, file Markdown ujian, maupun hasil pengerjaan yang dikirimkan ke server luar. Data Anda sepenuhnya aman.
Dibuat dengan ❤️ untuk kemudahan pengerjaan ujian.
